...
public class Vanguards extends JFrame {
    ...
    private static final Font TITLE_FONT = new Font("Serif", Font.BOLD | Font.ITALIC, 120);
    ...
    class DamageNumber {
        ...
        public DamageNumber(int v, int x, int y, Color c, String t) {
            this(v,x,y,c); text=t;
        }
        public void draw(Graphics2D g) {
    ...
    class TutorialScreen extends JPanel {
        public TutorialScreen() {
            ...
            String tutorialHtml = "<html><body style='width: 800px; color: #F5F5F5; font-family: SansSerif; font-size: 18px; line-height: 1.6; text-align: center;'>"
                    + "<b><font color='#D4AF37'>COMBAT:</font></b> Spend Energy to use abilities. String together attacks to build a Combo for extra damage.<br><br>"
                    + "<b><font color='#D4AF37'>STATS:</font></b><br>"
                    + "• <b>ATK</b> - Increases damage dealt. • <b>DEF</b> - Reduces damage taken.<br>"
                    + "• <b>SPD</b> - Increases Evasion and Parry chances. • <b>LUK</b> - Increases Critical Hit chance.<br><br>"
                    + "<b><font color='#D4AF37'>EQUIPMENT LIMITS:</font></b> You may equip up to <b>2 Weapons</b>, <b>4 Armors</b>, and <b>10 Relics</b>.<br><br>"
                    + "<b><font color='#D4AF37'>SHOP & BOUNTIES:</font></b> Slay enemies to earn Gold and complete Bounties. Use Gold in the Shop to buy gear, potions, and upgrades. The Shop refreshes every 3 battles.<br><br>"
                    + "<b><font color='#D4AF37'>MYSTERY BOX PITY SYSTEM:</font></b> If you hit 4 low-tier drops in a row, the 5th box is guaranteed to roll Epic rarity or higher."
                    + "</body></html>";
    ...
    class BattlePanel extends JPanel {
        ...
        private JPanel createMainActions() {
            ...
            for(String s : btns) {
                ...
                // Smart Button Formatting (Grey out if poor)
        ...
        private void refreshSubMenu(String key) {
            ...
            if(key.equals("ATTACKS")) {
                ...
                tabs.addTab("Weapons (" + player.equippedWeapons.size() + "/2)", wScroll);
                tabs.addTab("Armor (" + player.equippedArmors.size() + "/4)", aScroll);
                tabs.addTab("Relics (" + player.equippedRelics.size() + "/10)", rScroll);
            ...
            // Anchored bottom back button for all submenus
        ...
        private JPanel createItemCard(Equipment eq, boolean isEq, Player p) {
            ...
            eb.addActionListener(e -> {
                if(isEq) {
                    ...
                    else if(eq instanceof Weapon) p.equippedWeapons.remove(eq);
                    else if(eq instanceof Armor) p.equippedArmors.remove(eq);
                } else {
        ...
        private void buildSideShop() {
                    ...
                    if(p.gold >= it.price) {
                        ...
                        if(it instanceof Equipment) p.inventory.add(it);
                            ...
                            if(it.name.contains("Greater")) p.greaterPots++;
                                ...
                                Equipment dropped = generateProceduralEquipment(true); // pass true for pity check
            ...
            reroll.setPreferredSize(new Dimension(240, 40));
            reroll.setMaximumSize(new Dimension(240, 40));
            reroll.setAlignmentX(Component.CENTER_ALIGNMENT);
            reroll.setForeground(Color.CYAN);
            ...
            JScrollPane shopScroll = new JScrollPane(items);
            shopScroll.setBorder(null); shopScroll.setBackground(new Color(20, 20, 28)); shopScroll.getViewport().setOpaque(false);
            shopScroll.getVerticalScrollBar().setUnitIncrement(16);

            sideShopPanel.add(shopScroll, BorderLayout.CENTER);
            sideShopPanel.revalidate(); sideShopPanel.repaint();
        }
        ...
        private void executeAttack(String move) {
            ...
            p.onAnimComplete = () -> {
                ...
                int reducedDmg = finalDmg - enemy.def;
                ...
                for(Weapon w : p.equippedWeapons) {
                    if("Soul Syphon".equals(w.passive)) {
                ...
                if(enemy.hp <= 0) {
                    Timer t = new Timer(500, ev -> endEncounter());
                    t.setRepeats(false); t.start();
                } else {
                    Timer t = new Timer(500, ev -> endPlayerTurn());
                    t.setRepeats(false); t.start();
                }
            };
        }
        ...
        private void executeDefend() {
            ...
            for(Armor a : p.equippedArmors) {
                if("Knight's Resolve".equals(a.passive)) p.shield += 50;
            }
        ...
        private void executeRest() {
            ...
            int interest = (int)(p.gold * 0.05);
            p.gold += interest;

            p.energy = Math.min(p.maxEnergy, p.energy + 50);
            p.heal((int)(p.getTotalMaxHp() * 0.5));
        ...
        private void endEncounter() {
            ...
            if(state.bountyKills >= state.bountyTarget) {
                ...
                state.bountyKills = 0;
                state.bountyTarget += 2; state.bountyReward += 50;
            }
            ...
            Timer t = new Timer(1500, e -> {
                ...
                if(p.checkLevelUp(log)) {
                    dmgNums.add(new DamageNumber(0, 250, 150, Color.CYAN, "LEVEL UP!"));
                    setMenu("LEVEL_UP");
                } else setMenu("MAIN");
            });
            t.setRepeats(false); t.start();
        ...
        private void enemyTurn() {
            ...
            if(enemy.freezeTurns > 0) {
                enemy.freezeTurns--;
                log.append("[COMBAT] >> Enemy is FROZEN and skips their turn!\n");
            ...
            enemy.onAnimComplete = () -> {
                ...
                if(p.hp <= 0) {
                    log.append("\n[SYS] --- GAME OVER ---\n");
                    Timer deathTimer = new Timer(1500, dt -> {
                        gameLoop.stop();
        ...
        private Equipment generateProceduralEquipment(boolean fromMysteryBox) {
            ...
            double depthBonus = Math.min(0.25, state.encounters * 0.015);
            roll -= depthBonus;

            Rarity rarity;
            if(fromMysteryBox && mysteryBoxPity >= 4) {
                rarity = roll < 0.2 ? Rarity.GODLY : roll < 0.4 ? Rarity.MYTHIC : roll < 0.7 ? Rarity.LEGENDARY : Rarity.EPIC;
                mysteryBoxPity = 0;
            } else {
                ...
                if(fromMysteryBox) {
                    if(rarity == Rarity.COMMON || rarity == Rarity.RARE) mysteryBoxPity++;
                    else mysteryBoxPity = 0;
                }
            }

            int b = 2 + (state.encounters / 3) + rnd.nextInt(5);
            int m = (int)rarity.multiplier;
        ...
        private void refreshShop() {
            shopStock.clear();
            shopStock.add(generateProceduralEquipment(false));
            shopStock.add(generateProceduralEquipment(false));
        ...
        private void drawScene(Graphics2D g) {
            ...
            g.drawString("Total Gold: " + state.player.gold + "G", 20, 30);
            g.setColor(new Color(255,255,255,150));
            g.drawString("Encounter: " + state.encounters, 20, 50);
            g.drawString("Streak: " + state.player.winStreak, 20, 70);
            g.drawString("Bounty: " + state.bountyKills + "/" + state.bountyTarget, 20, 90);

            if(state.player.combo > 1) {
                int cSize = Math.min(45, 28 + state.player.combo * 2);
                g.setColor(state.player.combo > 5 ? Color.RED : XP_ORANGE);
                g.setFont(new Font("Impact", Font.ITALIC, cSize)); g.drawString(state.player.combo + "x COMBO", 20, 130);
            }
        ...
        private void drawBar(Graphics2D g, int x, int y, String label, double v, int m, Color c, int w, int h) {
            ...
            if(label.equals("ENERGY") && v > 0) {
                ...
                for(int i=0; i<3; i++) {
                    int px = x + (int)((tick * (2+i)) % (w * Math.max(0, Math.min(1.0, v / m))));
            ...
            // Clean Text Centered Inside Bar
            String text = label + (m > 0 && !label.equals("SHIELD") ? ": " + (int)v + " / " + m : ": " + (int)v);
            if(w > 500) text = label; // Boss logic

            g.setFont(w > 500 ? new Font("Serif", Font.BOLD, 18) : new Font("SansSerif", Font.BOLD, 12));
    ...
    class ClassSelect extends JPanel {
        public ClassSelect() {
            ...
            JPanel grid = new JPanel(new FlowLayout(FlowLayout.CENTER, 25, 25));
            grid.setOpaque(false);

            for(ClassType ct : ClassType.values()) {
                ClassButton b = new ClassButton("<html><center><b><font size='5'>" + ct.title + "</font></b><br><br><font size='3' color='gray'>" + ct.scaleDesc + "</font></center></html>", ct.color);
                b.setPreferredSize(new Dimension(160, 220));
                b.addActionListener(e -> { state.player = new Player(ct); ACCENT_COL = ct.color; battlePanel.initSession(); cards.show(mainContainer, "BATTLE"); });
                grid.add(b);
            }
            add(grid);
        }
